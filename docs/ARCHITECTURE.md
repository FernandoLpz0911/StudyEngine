...
cli/study.py
  └─ policy.select_next(subject)            pick concept + reason (review|new)
       ...
       └─ recall concept
            └─ recall.cards.as_flashcard(concept)             → front/back, self-rated
  └─ store.apply_rating(card, grade) → store.save(...)        FSRS schedules next due
  └─ dao.log_shown / log_answered                            every interaction logged
```
...
- **recall** — flashcard presentation for subjects without closed-form answers; the
  self-rating feeds the identical FSRS path.
